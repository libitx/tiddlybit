 ...
 * chunk can either be an OpCode or a binary chunk of data.
 * @class {Script}
 */
class Script {
  /**
  ...
  static fromASM(str) {
    ...
    for(let i = 0; i < tokens.length; i++) {
      ...
      if (token === '0') {
        chunks.push(OpCode.OP_0)
      }
      else if (token === '-1') {
        chunks.push(OpCode.OP_1NEGATE)
      }
      else {
        const chunk = OpCode[token] || binary.from(token, 'hex')
        chunks.push(chunk)
      }
    }
  ...
  static fromBuffer(data) {
    ...
    while(pos < data.length) {
      const byte = binary.readUInt8(data, pos)
      let buf, size, len;

      if (byte > 0 && byte < 76) {
        buf = binary.subarray(data, pos+1, pos+byte+1)
        len = buf.length + 1
      }
      else if (byte === 76) {
  ...
  toASM() {
    ...
    for(let i = 0; i < this.chunks.length; i++) {
      ...
      if (data instanceof OpCode) {
        if (data.code === 0 || data.code === OpCode.OP_1NEGATE.code) {
          chunks.push(data.code)
        } else {
          chunks.push(data.op)
        }
      } else {
